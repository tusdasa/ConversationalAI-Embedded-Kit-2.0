...
// #define CONFIG_CONTINUOUS_CONVERSATION_MODE true
...
recorder_pipeline_handle_t local_recorder_pipeline_open(void)
{
...
esp_err_t local_recorder_pipeline_run(recorder_pipeline_handle_t pipeline)
{
...
esp_err_t local_recorder_pipeline_stop(recorder_pipeline_handle_t pipeline)
{
    ESP_RETURN_ON_FALSE(pipeline != NULL, ESP_FAIL, TAG, "recorder pipeline not initialized");
...
esp_err_t local_recorder_pipeline_close(recorder_pipeline_handle_t pipeline)
{
    ESP_RETURN_ON_FALSE(pipeline != NULL, ESP_FAIL, TAG, "recorder pipeline not initialized");
...
audio_element_handle_t local_recorder_pipeline_get_raw_reader(recorder_pipeline_handle_t pipeline)
{
    return pipeline->raw_reader;
}

audio_pipeline_handle_t local_recorder_pipeline_get_pipeline(recorder_pipeline_handle_t pipeline)
{
    return pipeline->audio_pipeline;
};

int local_recorder_pipeline_read(recorder_pipeline_handle_t pipeline,char *buffer, int buf_size)
{
...
// {
//     ESP_LOGI(TAG, "%s", __func__);
//     player_pipeline_handle_t player_pipeline = heap_caps_calloc(1, sizeof(struct player_pipeline_t), MALLOC_CAP_SPIRAM | MALLOC_CAP_DEFAULT);
...
//     player_pipeline->i2s_stream_writer = create_player_i2s_stream(true);
...
//     audio_pipeline_register(player_pipeline->audio_pipeline, player_pipeline->i2s_stream_writer, "player_i2s");

//     player_pipeline->player_rsp = create_resample_stream(CODEC_SAMPLE_RATE, 1, I2S_SAMPLE_RATE, CHANNEL_NUM);
//     AUDIO_MEM_CHECK(TAG, player_pipeline->player_rsp, goto _exit_open);
//     audio_element_set_output_timeout(player_pipeline->player_rsp, portMAX_DELAY);
...
//     // audio_pipeline_register(player_pipeline->audio_pipeline, player_pipeline->player_rsp, "player_rsp");
//     const char *link_tag[]= {"player_raw", "player_rsp", "player_i2s"};
...
// esp_err_t local_player_pipeline_run(player_pipeline_handle_t player_pipeline)
// {
...
// esp_err_t local_player_pipeline_close(player_pipeline_handle_t player_pipeline)
// {
//     ESP_RETURN_ON_FALSE(player_pipeline != NULL, ESP_FAIL, TAG, "player pipeline not initialized");
//     if (player_pipeline->player_state == PIPE_STATE_RUNNING) {
